...
fn validate_action_config(
    ...
    match action_type {
        ...
        "schedule_nodes" => {
            ...
            if let Some(start_server_on_head_node) = config_obj.get("start_server_on_head_node") {
                if !start_server_on_head_node.is_boolean() {
                    return Err("'start_server_on_head_node' must be a boolean".to_string());
                }
            }
