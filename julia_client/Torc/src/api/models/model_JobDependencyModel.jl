# This file was generated by the Julia OpenAPI Code Generator
# Do not modify this file directly. Modify the OpenAPI specification instead.


@doc raw"""job_dependency_model
Represents a blocking relationship between two jobs in the job_blocked_by table.

    JobDependencyModel(;
        job_id=nothing,
        job_name=nothing,
        blocked_by_job_id=nothing,
        blocked_by_job_name=nothing,
        workflow_id=nothing,
    )

    - job_id::Int64 : The job that is blocked
    - job_name::String : The name of the job that is blocked
    - blocked_by_job_id::Int64 : The job that must complete first
    - blocked_by_job_name::String : The name of the job that must complete first
    - workflow_id::Int64 : The workflow containing both jobs
"""
Base.@kwdef mutable struct JobDependencyModel <: OpenAPI.APIModel
    job_id::Union{Nothing, Int64} = nothing
    job_name::Union{Nothing, String} = nothing
    blocked_by_job_id::Union{Nothing, Int64} = nothing
    blocked_by_job_name::Union{Nothing, String} = nothing
    workflow_id::Union{Nothing, Int64} = nothing

    function JobDependencyModel(job_id, job_name, blocked_by_job_id, blocked_by_job_name, workflow_id, )
        o = new(job_id, job_name, blocked_by_job_id, blocked_by_job_name, workflow_id, )
        OpenAPI.validate_properties(o)
        return o
    end
end # type JobDependencyModel

const _property_types_JobDependencyModel = Dict{Symbol,String}(Symbol("job_id")=>"Int64", Symbol("job_name")=>"String", Symbol("blocked_by_job_id")=>"Int64", Symbol("blocked_by_job_name")=>"String", Symbol("workflow_id")=>"Int64", )
OpenAPI.property_type(::Type{ JobDependencyModel }, name::Symbol) = Union{Nothing,eval(Base.Meta.parse(_property_types_JobDependencyModel[name]))}

function OpenAPI.check_required(o::JobDependencyModel)
    o.job_id === nothing && (return false)
    o.job_name === nothing && (return false)
    o.blocked_by_job_id === nothing && (return false)
    o.blocked_by_job_name === nothing && (return false)
    o.workflow_id === nothing && (return false)
    true
end

function OpenAPI.validate_properties(o::JobDependencyModel)
    OpenAPI.validate_property(JobDependencyModel, Symbol("job_id"), o.job_id)
    OpenAPI.validate_property(JobDependencyModel, Symbol("job_name"), o.job_name)
    OpenAPI.validate_property(JobDependencyModel, Symbol("blocked_by_job_id"), o.blocked_by_job_id)
    OpenAPI.validate_property(JobDependencyModel, Symbol("blocked_by_job_name"), o.blocked_by_job_name)
    OpenAPI.validate_property(JobDependencyModel, Symbol("workflow_id"), o.workflow_id)
end

function OpenAPI.validate_property(::Type{ JobDependencyModel }, name::Symbol, val)





end
